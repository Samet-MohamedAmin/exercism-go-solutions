...
func TestRomanNumerals(t *testing.T) {
	...
	for _, test := range tc[4:] {
		actual, err := ToRomanNumeral(test.arabic)
		if err == nil && test.hasError {
			t.Fatalf("ToRomanNumeral(%d) should return an error.", test.arabic)
			continue
		}
		if err != nil && !test.hasError {
			var _ error = err
			t.Fatalf("ToRomanNumeral(%d) should not return an error.", test.arabic)
			continue
		}
		if !test.hasError && actual != test.roman {
			t.Fatalf("ToRomanNumeral(%d): %s, expected %s", test.arabic, actual, test.roman)
		}
	}
}
