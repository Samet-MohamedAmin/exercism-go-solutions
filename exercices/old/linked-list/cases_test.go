...
func popFront(expected interface{}, expectedErr error) checkedAction {
	return func(t *testing.T, ll *List) {
		v, err := ll.PopFront()
		if err != expectedErr {
			t.Fatalf("PopFront() returned wrong, expected no error, got= %v", err)
		}

		if expectedErr == nil && v != expected {
			t.Fatalf("PopFront() returned wrong, expected= %v, got= %v", expected, v)
		}
	}
}
...
func popBack(expected interface{}, expectedErr error) checkedAction {
	return func(t *testing.T, ll *List) {
		v, err := ll.PopBack()
		if err != expectedErr {
			t.Fatalf("PopBack() returned wrong, expected no error, got= %v", err)
		}

		if expectedErr == nil && v != expected {
			t.Fatalf("PopBack() returned wrong, expected= %v, got= %v", expected, v)
		}
	}
}
