...
func checkDoublyLinkedList(t *testing.T, ll *List, expected []interface{}) {
	...
	for ; elem != nil && idx < len(expected); elem, count, idx = elem.Next(), count+1, idx+1 {
		if elem.Val != expected[idx] {
			t.Fatalf("wrong value from %d-th element, expected= %v, got= %v", idx, expected[idx], elem.Val)
		}
	}
	if !(elem == nil && idx == len(expected)) {
		t.Fatalf("expected %d elements, got= %d", len(expected), count)
	}
	...
	case ll.First() != nil && ll.Last() != nil && ll.First().Next() == nil: // 1 element
		...
		if !valid {
			t.Fatalf("expected to only have 1 element and no links, got= %v", ll.debugString())
		}
	}

	// >1 element
	if ll.First().Prev() != nil {
		t.Fatalf("expected Head.Prev() == nil, got= %v", ll.First().Prev())
	}
	...
	for idx := 0; cur != nil; idx++ {
		if !(prev.Next() == cur && cur.Prev() == prev) {
			t.Fatalf("%d-th element's links is wrong", idx)
		}
	...
	if ll.Last().Next() != nil {
		t.Fatalf("expected Last().Next() == nil, got= %v", ll.Last().Next())
	}
}
